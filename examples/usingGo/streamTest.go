...
func handleStreamInput(input string) map[string]interface{} {
	...
	output, err := api.EvalActivity(actStream["agg1"], tmp)
	...
	output, err = api.EvalActivity(actStream["agg2"], tmp)
...
func runTimerActivitiesStream(ctx context.Context, inputs map[string]interface{}) (map[string]interface{}, error) {
	...
	// fmt.Println("[@Timer]$ Sending Data: ", input)
	// fmt.Println()
	...
	// fmt.Println("[@Timer]$ Received Response : ", output)
	// fmt.Println()
